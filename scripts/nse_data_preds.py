...
fig.add_trace(
    go.Ohlc(
        x=df.Date, open=df.Open, high=df.High, low=df.Losw, close=df.Close, name="Price"
    ),
    row=1,
    col=1,
)
...
df = df.iloc[
    33:
]  # Because of moving averag    If the version of a package is being increased, please remember to return the build/number back to 0.
...
fig = make_subplots(rows=2, cols=1)
fig.add_trace(
    go.Scatter(x=df.Date, y=df.Close, name="Truth", marker_color="LightSkyBlue"),
    row=1,
    col=1,
)
